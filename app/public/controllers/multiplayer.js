...
controllers.controller('multiplayerController', function($scope, $stateParams, socket, $state, $mdDialog, $timeout, $http, $interval) {
  ...
  $scope.play = function() {
    if (canvas && canvas.ready) {
      ...
      try {
        ...
        socket.emit('multiplayer:play', output);
  ...
  $scope.quit = function() {
    if (confirm('Leave this game?')) {
      socket.emit('multiplayer:quit', $scope.game.id);
      if (game) {
        canvas.game.destroy();
        canvas = null
        game = null;
  ...
  $scope.addPoint = function(prop) {
    socket.emit('multiplayer:command', {
      type: 'add-point',
      point: prop
    });
  };
  ...
  function init() {
    socket.on('multiplayer:state', function(data) {
      if (!game) {
        ...
        data.game.players.forEach(function(options) {
          ...
          if (!player) {
            ...
            canvas.character.sprite.bringToTop();
          } else {
            player.update(options);
          }
        });
    ...
    socket.on('multiplayer:playback', function(data) {
      if (data) {
